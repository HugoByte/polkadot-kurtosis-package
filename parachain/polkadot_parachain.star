...
def run_acala(plan, args):
    exec_command = ["--chain=/app/acala-raw.json", "--collator", "--rpc-external",  "--rpc-cors=all", "--rpc-methods=unsafe",  "--tmp" , "--instant-sealing", "--", "--chain=/app/rococo-local.json", "--execution=wasm"]
...
def run_centriguge(plan, args):
    exec_command = ["--chain=/app/centrifuge-raw-spec.json", "--collator", "--rpc-external", "--rpc-cors=all", "--rpc-methods=unsafe", "--tmp", "--", "--chain=/app/rococo-local.json", "--execution=wasm"]
